...
class NarrativeTonePass(BasePass):
    pass_name = "l17_narrative_tone"
    categories = ["L17"]
    ...
    def run(
        ...
        if batch_enabled and batch_max_items > 1:
            ...
            for scene in scene_list:
                ...
                payload = {
                    "task": "l17_narrative_tone_check",
                    "scene": {"title": scene.scene_title, "content": scene.content_raw[:1500]},
                    "instruction": (
                        "检查同一场景内叙述语气/语域是否出现无理由跳变。"
                        '仅返回 JSON: {"ok":true,"issues":[...],"intermediate_data":{"tone":"..."}}'
                    ),
                }
    ...
    def _build_issues(self, scene: SceneMeta, items: list[dict[str, Any]]) -> list[Issue]:
        ...
        for idx, item in enumerate(items, start=1):
            out.append(
                Issue(
                    issue_id=str(item.get("issue_id", f"L17-{scene.node_id[:6]}-{idx}")),
                    category="L17",
                    pass_name=self.pass_name,
                    severity=str(item.get("severity", "warning")),
                    title=str(item.get("title", "叙述语气跳变")),
                    ...
                    suggestion=str(item.get("suggestion", "统一叙述语域或补充语气变化触发点")),
