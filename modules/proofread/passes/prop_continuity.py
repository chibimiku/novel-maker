...
class PropContinuityPass(BasePass):
    pass_name = "l9_prop_continuity"
    categories = ["L9"]
    ...
    def run(
        ...
        if batch_enabled and batch_max_items > 1:
            ...
            for scene in scene_list:
                ...
                payload = {
                    "task": "l9_prop_continuity_check",
                    "previous_scene": None
                    if prev is None
                    else {"title": prev.scene_title, "content": prev.content_raw[:1000]},
                    "current_scene": {"title": scene.scene_title, "content": scene.content_raw[:1000]},
                    "instruction": (
                        "检查重要物品/道具是否凭空出现、消失或不合理转移。"
                        '仅返回 JSON: {"ok":true,"issues":[...],"intermediate_data":{"props":[...]}}'
                    ),
                }
            ...
            for scene in scene_list:
                ...
                payload = {
                    ...
                    else {"title": prev_scene.scene_title, "content": prev_scene.content_raw[:1000]},
                    "current_scene": {"title": scene.scene_title, "content": scene.content_raw[:1000]},
                    "instruction": (
                        "检查重要物品/道具是否凭空出现、消失或不合理转移。"
                        '仅返回 JSON: {"ok":true,"issues":[...],"intermediate_data":{"props":[...]}}'
    ...
    def _build_issues(self, scene: SceneMeta, items: list[dict[str, Any]]) -> list[Issue]:
        ...
        for idx, item in enumerate(items, start=1):
            out.append(
                Issue(
                    issue_id=str(item.get("issue_id", f"L9-{scene.node_id[:6]}-{idx}")),
                    category="L9",
                    pass_name=self.pass_name,
                    severity=str(item.get("severity", "warning")),
                    title=str(item.get("title", "物品/道具矛盾")),
                    ...
                    suggestion=str(item.get("suggestion", "补充道具转移/获取/丢失过程")),
