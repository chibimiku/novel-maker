...
class LightingPass(BasePass):
    pass_name = "l8_lighting"
    categories = ["L8"]
    ...
    def run(
        ...
        if batch_enabled and batch_max_items > 1:
            ...
            for scene in scene_list:
                ...
                payload = {
                    "task": "l8_lighting_check",
                    "scene": {"title": scene.scene_title, "summary": scene.summary_raw, "content": scene.content_raw[:1500]},
                    "instruction": (
                        "检测时间与光照描写是否矛盾。"
                        '仅返回 JSON: {"ok":true,"issues":[...],"intermediate_data":{"lighting":"..."}}'
                    ),
                }
    ...
    def _build_issues(self, scene: SceneMeta, items: list[dict[str, Any]]) -> list[Issue]:
        ...
        for idx, item in enumerate(items, start=1):
            out.append(
                Issue(
                    issue_id=str(item.get("issue_id", f"L8-{scene.node_id[:6]}-{idx}")),
                    category="L8",
                    pass_name=self.pass_name,
                    severity=str(item.get("severity", "warning")),
                    title=str(item.get("title", "光照矛盾")),
                    ...
                    suggestion=str(item.get("suggestion", "调整时间标记或补充光源说明")),
