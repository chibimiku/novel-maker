...
class PovConsistencyPass(BasePass):
    pass_name = "l16_pov_consistency"
    categories = ["L16"]
    ...
    def run(
        ...
        if batch_enabled and batch_max_items > 1:
            ...
            for scene in scene_list:
                ...
                payload = {
                    "task": "l16_pov_consistency_check",
                    "scene": {"title": scene.scene_title, "content": scene.content_raw[:1500]},
                    "instruction": (
                        "判断是否存在 POV 视角跳跃或越界信息。"
                        '仅返回 JSON: {"ok":true,"issues":[...],"intermediate_data":{"pov":"..."}}'
                    ),
                }
    ...
    def _build_issues(self, scene: SceneMeta, items: list[dict[str, Any]]) -> list[Issue]:
        ...
        for idx, item in enumerate(items, start=1):
            out.append(
                Issue(
                    issue_id=str(item.get("issue_id", f"L16-{scene.node_id[:6]}-{idx}")),
                    category="L16",
                    pass_name=self.pass_name,
                    severity=str(item.get("severity", "warning")),
                    title=str(item.get("title", "POV视角跳跃")),
                    ...
                    suggestion=str(item.get("suggestion", "统一视角或增加明确切换标记")),
