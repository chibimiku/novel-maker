...
class RelationshipsPass(BasePass):
    pass_name = "l3_relationships"
    categories = ["L3"]
        ...
        if batch_enabled and batch_max_items > 1:
            ...
            for scene in scene_list:
                ...
                payload = {
                    ...
                    "instruction": (
                        "检查对话中的称呼与角色关系是否一致。"
                        '仅返回 JSON: {"ok":true,"issues":[...],"intermediate_data":{"dialogue_segments":[...]}}'
                    ),
                }
    ...
    def _build_issues(self, scene: SceneMeta, issue_items: list[dict[str, Any]]) -> list[Issue]:
        ...
        for idx, item in enumerate(issue_items, start=1):
            out.append(
                Issue(
                    issue_id=str(item.get("issue_id", f"L3-{scene.node_id[:6]}-{idx}")),
                    category="L3",
                    pass_name=self.pass_name,
                    severity=str(item.get("severity", "warning")),
                    title=str(item.get("title", "关系/称呼错误")),
                    ...
                    suggestion=str(item.get("suggestion", "调整称呼以符合人物关系设定")),
