...
class BehaviorLogicPass(BasePass):
    pass_name = "l13_behavior_logic"
    categories = ["L13"]
    ...
    def run(
        ...
        if batch_enabled and batch_max_items > 1:
            ...
            for scene in scene_list:
                ...
                payload = {
                    "task": "l13_behavior_logic_check",
                    "previous_scene": None
                    if prev is None
                    else {"title": prev.scene_title, "summary": prev.summary_raw, "content": prev.content_raw[:900]},
                    "current_scene": {"title": scene.scene_title, "summary": scene.summary_raw, "content": scene.content_raw[:900]},
                    "instruction": (
                        "检查角色行为与情绪/动机是否出现断裂式跳变。"
                        '仅返回 JSON: {"ok":true,"issues":[...],"intermediate_data":{"emotion_tags":[...]}}'
                    ),
                }
            ...
            for scene in scene_list:
                ...
                payload = {
                    ...
                    else {"title": prev_scene.scene_title, "summary": prev_scene.summary_raw, "content": prev_scene.content_raw[:900]},
                    "current_scene": {"title": scene.scene_title, "summary": scene.summary_raw, "content": scene.content_raw[:900]},
                    "instruction": (
                        "检查角色行为与情绪/动机是否出现断裂式跳变。"
                        '仅返回 JSON: {"ok":true,"issues":[...],"intermediate_data":{"emotion_tags":[...]}}'
    ...
    def _build_issues(self, scene: SceneMeta, items: list[dict[str, Any]]) -> list[Issue]:
        ...
        for idx, item in enumerate(items, start=1):
            out.append(
                Issue(
                    issue_id=str(item.get("issue_id", f"L13-{scene.node_id[:6]}-{idx}")),
                    category="L13",
                    pass_name=self.pass_name,
                    severity=str(item.get("severity", "warning")),
                    title=str(item.get("title", "角色行为逻辑断裂")),
                    ...
                    suggestion=str(item.get("suggestion", "补充心理过渡或触发事件")),
