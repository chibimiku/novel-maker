...
class WeatherEnvPass(BasePass):
    pass_name = "l7_weather_env"
    categories = ["L7"]
    ...
    def run(
        ...
        if batch_enabled and batch_max_items > 1:
            ...
            for scene in scene_list:
                ...
                payload = {
                    "task": "l7_weather_env_check",
                    "previous_scene": None
                    if prev is None
                    else {"title": prev.scene_title, "summary": prev.summary_raw, "content": prev.content_raw[:1200]},
                    "current_scene": {"title": scene.scene_title, "summary": scene.summary_raw, "content": scene.content_raw[:1200]},
                    "instruction": (
                        "检测天气/环境前后是否矛盾。"
                        '仅返回 JSON: {"ok":true,"issues":[...],"intermediate_data":{"weather":{"prev":"...","curr":"..."}}}'
                    ),
                }
            ...
            for scene in scene_list:
                ...
                payload = {
                    ...
                    else {"title": prev_scene.scene_title, "summary": prev_scene.summary_raw, "content": prev_scene.content_raw[:1200]},
                    "current_scene": {"title": scene.scene_title, "summary": scene.summary_raw, "content": scene.content_raw[:1200]},
                    "instruction": (
                        "检测天气/环境前后是否矛盾。"
                        '仅返回 JSON: {"ok":true,"issues":[...],"intermediate_data":{"weather":{"prev":"...","curr":"..."}}}'
    ...
    def _build_issues(self, scene: SceneMeta, items: list[dict[str, Any]]) -> list[Issue]:
        ...
        for idx, item in enumerate(items, start=1):
            out.append(
                Issue(
                    issue_id=str(item.get("issue_id", f"L7-{scene.node_id[:6]}-{idx}")),
                    category="L7",
                    pass_name=self.pass_name,
                    severity=str(item.get("severity", "warning")),
                    title=str(item.get("title", "天气/环境矛盾")),
                    ...
                    suggestion=str(item.get("suggestion", "补充天气变化过渡或统一环境描写")),
